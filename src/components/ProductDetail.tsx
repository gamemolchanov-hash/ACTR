  ...
  return (
    <Box sx={{ maxWidth: 1300, mx: 'auto', px: 2, py: { xs: 2, md: 3 } }}>
      ...
      <Grid container spacing={4} alignItems="flex-start">
        {/* Left: Image gallery — инлайн-свайп с точками + полноэкранный просмотр
            (пинч-зум, свайп, стрелки/клавиатура) — порт галереи витрины .ru */}
        <Grid item xs={12} md={5} data-testid="sf-product-gallery">
          <ProductGallery
            images={images}
            index={selectedImageIndex}
            alt={product.name}
            emptyLabel={product.sku}
            onIndexChange={setSelectedImageIndex}
            onOpen={() => setLightboxOpen(true)}
            labels={{
              region: t('product.galleryRegion'),
              carousel: t('product.galleryCarousel'),
              dots: t('product.galleryDots'),
              photo: t('product.galleryPhoto'),
            }}
          />
          <FullscreenGallery
            open={lightboxOpen}
            ...
            onIndexChange={setSelectedImageIndex}
            labels={{
              close: t('product.galleryClose'),
              prev: t('product.galleryPrev'),
              next: t('product.galleryNext'),
            }}
          />
        </Grid>
